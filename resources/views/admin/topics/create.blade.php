        ...
        <div class="card-body">
            <form action="{{ route('admin.topics.store') }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('topic_code') is-invalid @enderror" id="topic_code" name="topic_code" value="{{ old('topic_code') }}">
                    @error('topic_code')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // If unit_id is pre-selected (from request parameter)
        @if(request('unit_id'))
            const unitId = {{ request('unit_id') }};
            document.querySelectorAll('#unit_id option').forEach(option => {
