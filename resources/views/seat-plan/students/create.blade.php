        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header bg-primary text-white">
                    <h5 class="mb-0">
                        <i class="fas fa-plus-circle mr-2"></i> Create New Student
                    </h5>
                </div>
                <div class="card-body">
                    <form action="{{ route('students.store') }}" method="POST">
                        ...
                        <div class="form-group">
                            <div class="custom-control custom-switch">
                                <input type="checkbox" class="custom-control-input" id="has_special_needs" name="has_special_needs" value="1" {{ old('has_special_needs') ? 'checked' : '' }}>
                                <label class="custom-control-label" for="has_special_needs">Has Special Needs</label>
                            </div>
                        </div>

                        <div class="form-group special-needs-details" style="{{ old('has_special_needs') ? '' : 'display: none;' }}">
                            <label for="special_needs_details">Special Needs Details</label>
                            <textarea class="form-control @error('special_needs_details') is-invalid @enderror" id="special_needs_details" name="special_needs_details" rows="3">{{ old('special_needs_details') }}</textarea>
...
<script>
    $(document).ready(function() {
        ...
        // Auto-generate roll number based on course and year
        $('#course_id, #year').change(function() {
            if ($('#roll_number').val() === '') {
                let courseId = $('#course_id').val();
                let year = $('#year').val();
                
                if (courseId && year) {
                    let courseCode = $('#course_id option:selected').text().match(/\(([^)]+)\)/)[1];
                    let currentYear = new Date().getFullYear().toString().substr(-2);
                    
                    // Generate a random 3-digit number
                    let randomNum = Math.floor(Math.random() * 900) + 100;
                    
                    $('#roll_number').val(courseCode + currentYear + year + randomNum);
                }
            }
        });
    });
