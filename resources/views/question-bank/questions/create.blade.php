...
<div class="card shadow fade-in">
    ...
    <div class="card-body">
        <form action="{{ route('question-bank.questions.store') }}" method="POST" id="question-form">
            @csrf
            ...
            <div class="row">
                ...
                <div class="col-md-4 mb-3">
                    ...
                    <select class="form-select @error('unit_id') is-invalid @enderror" id="unit_id" name="unit_id" required {{ old('subject_id') ? '' : 'disabled' }}>
                        <option value="">Select Unit</option>
                        @if(old('subject_id') && isset($units))
                ...
                <div class="col-md-4 mb-3">
                    ...
                    <select class="form-select @error('topic_id') is-invalid @enderror" id="topic_id" name="topic_id" required {{ old('unit_id') ? '' : 'disabled' }}>
                        <option value="">Select Topic</option>
                        @if(old('unit_id') && isset($topics))
                ...
                <div class="col-md-4 mb-3">
                    ...
                    <select class="form-select @error('question_type_id') is-invalid @enderror" id="question_type_id" name="question_type_id" required>
                        ...
                        @foreach($questionTypes as $type)
                            <option value="{{ $type->id }}" data-structure="{{ json_encode($type->structure) }}" {{ old('question_type_id') == $type->id ? 'selected' : '' }}>
                                {{ $type->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-4 mb-3">
                    ...
                    <select class="form-select @error('blooms_taxonomy_id') is-invalid @enderror" id="blooms_taxonomy_id" name="blooms_taxonomy_id">
                        ...
                        @foreach($bloomsLevels as $level)
                            <option value="{{ $level->id }}" {{ old('blooms_taxonomy_id') == $level->id ? 'selected' : '' }}>
                                {{ $level->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div id="dynamic-fields-container">
                ...
                @if(old('question_type_id'))
                    <div id="dynamic-fields-placeholder">
                        <!-- Fields will be re-populated from old input if validation fails -->
                    </div>
                @endif
            </div>
            ...
            <div class="d-flex justify-content-end">
                <a href="{{ route('question-bank.questions.index') }}" class="btn btn-secondary me-2">
...
<script>
            ...
            switch (field.type) {
                case 'text':
                    fieldHtml = `
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('${key}') is-invalid @enderror" id="${key}" name="${key}" value="{{ old('${key}') }}" ${field.required ? 'required' : ''}>
                ...
                case 'textarea':
                    fieldHtml = `
                        <div class="mb-3">
                            <label for="${key}" class="form-label">${field.label} ${field.required ? '<span class="text-danger">*</span>' : ''}</label>
                            <textarea class="form-control @error('${key}') is-invalid @enderror" id="${key}" name="${key}" rows="3" ${field.required ? 'required' : ''}>{{ old('${key}') }}</textarea>
                            ${field.description ? `<small class="form-text text-muted">${field.description}</small>` : ''}
                            @error('${key}')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                ...
                case 'checkbox':
                    fieldHtml = `
                        <div class="mb-3 form-check">
                            <input type="checkbox" class="form-check-input" id="${key}" name="${key}" value="1" {{ old('${key}') ? 'checked' : '' }}>
                ...
                case 'number':
                    fieldHtml = `
                        <div class="mb-3">
                            <label for="${key}" class="form-label">${field.label} ${field.required ? '<span class="text-danger">*</span>' : ''}</label>
                            <input type="number" class="form-control @error('${key}') is-invalid @enderror" id="${key}" name="${key}" value="{{ old('${key}') }}" ${field.min ? `min="${field.min}"` : ''} ${field.max ? `max="${field.max}"` : ''} ${field.required ? 'required' : ''}>
                            ${field.description ? `<small class="form-text text-muted">${field.description}</small>` : ''}
                            @error('${key}')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
    ...
    $(document).ready(function() {
        // Trigger change event to populate fields if question type is already selected (e.g., after validation error)
