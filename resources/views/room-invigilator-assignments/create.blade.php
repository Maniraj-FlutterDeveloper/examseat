...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="page-title">Assign Invigilator to Room</h1>
    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('room-invigilator-assignments.store') }}" method="POST">
                @csrf
                
                <div class="row mb-3">
                    <div class="col-md-4">
                        <label for="exam_date" class="form-label">Exam Date <span class="text-danger">*</span></label>
                        <input type="date" class="form-control @error('exam_date') is-invalid @enderror" id="exam_date" name="exam_date" value="{{ old('exam_date', request('exam_date')) }}" required>
                        @error('exam_date')
                    ...
                    <div class="col-md-4">
                        ...
                        <select class="form-select @error('time_slot') is-invalid @enderror" id="time_slot" name="time_slot" required>
                            ...
                            <option value="Morning (9:00 AM - 12:00 PM)" {{ old('time_slot', request('time_slot')) == 'Morning (9:00 AM - 12:00 PM)' ? 'selected' : '' }}>Morning (9:00 AM - 12:00 PM)</option>
                            <option value="Afternoon (2:00 PM - 5:00 PM)" {{ old('time_slot', request('time_slot')) == 'Afternoon (2:00 PM - 5:00 PM)' ? 'selected' : '' }}>Afternoon (2:00 PM - 5:00 PM)</option>
                            <option value="Evening (6:00 PM - 9:00 PM)" {{ old('time_slot', request('time_slot')) == 'Evening (6:00 PM - 9:00 PM)' ? 'selected' : '' }}>Evening (6:00 PM - 9:00 PM)</option>
                        </select>
                        @error('time_slot')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="col-md-4">
                        ...
                        <select class="form-select @error('is_chief_invigilator') is-invalid @enderror" id="is_chief_invigilator" name="is_chief_invigilator" required>
                            <option value="0" {{ old('is_chief_invigilator') == '0' ? 'selected' : '' }}>Assistant Invigilator</option>
                            <option value="1" {{ old('is_chief_invigilator') == '1' ? 'selected' : '' }}>Chief Invigilator</option>
                        </select>
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select @error('room_id') is-invalid @enderror" id="room_id" name="room_id" required {{ old('block_id') ? '' : 'disabled' }}>
                            <option value="">Select Room</option>
                            @if(old('block_id'))
                                @foreach($rooms->where('block_id', old('block_id')) as $room)
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('invigilator_id') is-invalid @enderror" id="invigilator_id" name="invigilator_id" required>
                        ...
                        @foreach($invigilators as $invigilator)
                            <option value="{{ $invigilator->id }}" data-department="{{ $invigilator->department }}" {{ old('invigilator_id', request('invigilator_id')) == $invigilator->id ? 'selected' : '' }}>
                ...
                <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                    ...
                    <button type="submit" class="btn btn-primary">Assign Invigilator</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
            ...
            if (blockId) {
                ...
                fetch(`/api/blocks/${blockId}/rooms`)
                    ...
                    .then(rooms => {
                        ...
                        // If room_id is provided in the request, select it
                        const requestedRoomId = '{{ request('room_id') }}';
                        if (requestedRoomId) {
        ...
        // Initialize rooms if block is selected
